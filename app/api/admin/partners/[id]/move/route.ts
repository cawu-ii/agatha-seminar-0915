...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (idx === -1) {
      return NextResponse.json({ error: "找不到該筆夥伴" }, { status: 404 });
    }

    const swapIdx = direction === "up" ? idx - 1 : idx + 1;
    if (swapIdx < 0 || swapIdx >= partners.length) {
      return NextResponse.json({ ok: true }); // already at the edge, no-op
    }

    const current = partners[idx];
    const neighbor = partners[swapIdx];
    await prisma.$transaction([
      prisma.partner.update({ where: { id: current.id }, data: { sortOrder: neighbor.sortOrder } }),
      prisma.partner.update({ where: { id: neighbor.id }, data: { sortOrder: current.sortOrder } }),
    ]);

    return NextResponse.json({ ok: true });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error("[api/admin/partners/:id/move] failed", err);
