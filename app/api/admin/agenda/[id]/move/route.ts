...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (idx === -1) {
      return NextResponse.json({ error: "找不到該筆議程" }, { status: 404 });
    }
    ...
    await prisma.$transaction([
      prisma.agendaItem.update({ where: { id: current.id }, data: { sortOrder: neighbor.sortOrder } }),
      prisma.agendaItem.update({ where: { id: neighbor.id }, data: { sortOrder: current.sortOrder } }),
    ]);

    return NextResponse.json({ ok: true });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error("[api/admin/agenda/:id/move] failed", err);
