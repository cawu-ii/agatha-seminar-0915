...
- **Hero 圖片已改用實際的 banner 設計稿（2026/08/10）**：原本這裡用的是一張抽象的裝飾圖（`hero-bg.png`，球體/網格圖形），上面另外疊了一段程式碼寫死的標語／標題／日期／標籤文字。Lindy 提供了「EMERGENECE DAY 2026」這份完整設計好的活動主視覺（跟落地頁 `/admin/banner` 上傳的 banner 是同一份圖檔），裡面已經把標題、日期、地點、報名按鈕都畫進圖片本身了——如果保留原本寫死的文字疊在圖片上會變成重複顯示，跟同一天稍早修的落地頁「兩塊 banner」是同一種問題。改成直接用這張設計稿整張取代原本寫死的文字內容，圖片存在 [`public/email-assets/hero-banner.jpg`](public/email-assets/hero-banner.jpg)（1080×1350，跟落地頁手機版 banner 同一份檔案），舊的 `hero-bg.png` 已刪除，不再使用。「報名成功」的綠色徽章保留在圖片下方，不屬於設計稿內容，是每次寄信才會顯示的動態確認狀態。
- **「報名成功」勾勾圖示跑版已修復（2026/08/10）**：原本用的打勾符號是 Unicode 的「粗體勾勾」（✔，U+2714），這個字元在部分信箱（實測是 Gmail）會被當成表情符號处理，用比文字大很多的圖示單獨佔一行顯示，把「報名成功」擠到下一行、整個徽章看起來變形。改用另一個「純文字樣式」的勾勾字元（✓，U+2713，較少被信箱當成表情符號），並明確指定該元素的 `line-height` 與 `white-space:nowrap`，強制勾勾跟文字維持在同一行，不受字元本身的顯示行為影響。
...
| 確認信 Hero 圖片改用真實 banner 設計稿＋修復「報名成功」勾勾跑版 | `done` | 2026/08/10：把確認信裡原本抽象裝飾圖＋寫死文字的 Hero，改成直接用 Lindy 的「EMERGENECE DAY 2026」banner 設計稿整張取代（跟落地頁 banner 同一份檔案，避免文字與圖片重複顯示，同一天稍早落地頁「兩塊 banner」問題的同類修法）；另外修復「報名成功」綠色徽章的勾勾符號在部分信箱（Gmail 實測）被當表情符號渲染、擠成兩行跑版的問題，改用文字樣式的勾勾字元並加上 `white-space:nowrap` 強制同行顯示。已用本機渲染＋瀏覽器驗證圖片正確載入、徽章單行顯示 |
