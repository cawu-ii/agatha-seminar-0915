...
//
// Also backfills `category` on already-seeded rows (openspec:
// add-speaker-partner-upload) by matching on `name` - this makes the seed
// idempotent per-row (find-or-create-then-sync-category) instead of the
// previous "bail out if any row exists" guard, so re-running it after this
// change safely categorizes existing installs without duplicating rows.
import { PrismaClient, PartnerCategory } from "@prisma/client";

const prisma = new PrismaClient();

const PARTNERS: Array<{ name: string; description: string; logoUrl: string; category: PartnerCategory }> = [
  {
    name: "今晧實業",
    logoUrl: "/images/asset-9ea1ca600b.png",
    category: PartnerCategory.HOST,
    description:
  ...
  },
  {
    ...
    logoUrl: "/images/asset-9ea1ca600b.png",
    category: PartnerCategory.HOST,
    description: "本次論壇主辦單位，Agatha 企業級 Agentic AI 平台開發團隊，致力於雲地整合與治理，協助製造業將 AI 嵌入產、銷、人、發、財五大流程。",
  },
  {
    name: "金屬工業研究發展中心",
    logoUrl: "/images/asset-ee461bf2f8.png",
    category: PartnerCategory.COORGANIZER,
    description:
...
async function main() {
  ...
  for (let i = 0; i < PARTNERS.length; i++) {
    ...
    if (existing.category !== partner.category) {
      await prisma.partner.update({ where: { id: existing.id }, data: { category: partner.category } });
      categorized++;
    }
  }

  console.log(`Created ${created} new partner(s), backfilled category on ${categorized} existing partner(s).`);
}
